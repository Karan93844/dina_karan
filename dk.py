...
# Fixed: Added a valid, direct streaming MP3 link for SoundHelix
st.audio("https://soundhelix.com", loop=True)

if st.button("Generate ✨"):
    c1, c2 = st.columns(2)
    # Fixed: Using the proper official API format for DiceBear to render real avatars
    c1.image(f"https://dicebear.com{wisher}", caption=wisher)
    c2.image(f"https://dicebear.com{receiver}", caption=f"{receiver} 🎉")
    
    # Fixed Indentation (exactly 4 spaces) and kept the highly accessible Zephyr model
    ...
    with col_text, st.spinner("Writing..."):
        ...
        try:
            ...
            # Fixed index reference structure
